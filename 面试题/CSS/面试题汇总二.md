...
CSS3 中的盒模型有以下两种：标准盒子模型、IE 盒子模型 ![img](./images\4544d45b5a0c47a58c0c33a7d8fbac09tplv-k3u1fbpfcp-zoom-in-crop-mark3024000.webp) ![img](./images\4040de9fef1a49f4ae0ae66039edcfe0tplv-k3u1fbpfcp-zoom-in-crop-mark3024000.webp) 盒模型都是由四个部分组成的，分别是 margin、border、padding 和 content。
...
对于图片来说，为了保证其不失真，1 个图片像素至少要对应一个物理像素，假如原始图片是 500300 像素，那么在 3 倍屏上就要放一个 1500900 像素的图片才能保证 1 个物理像素至少对应一个图片像素，才能不失真。 ![img](./images\5f1da1cbae9b45528cc2b33f74eb32f2tplv-k3u1fbpfcp-zoom-in-crop-mark3024000.webp) 当然，也可以针对所有屏幕，都只提供最高清图片。虽然低密度屏幕用不到那么多图片像素，而且会因为下载多余的像素造成带宽浪费和下载延迟，但从结果上说能保证图片在所有屏幕上都不会失真。
...
预处理器，其实就是 CSS 世界的“轮子”。预处理器支持我们写一种类似 CSS、但实际并不是 CSS 的语言，然后把它编译成 CSS 代码： ![img](./images\3d58c5313e884e38b1545a5896613250tplv-k3u1fbpfcp-zoom-in-crop-mark3024000.webp) 那为什么写 CSS 代码写得好好的，偏偏要转去写“类 CSS”呢？这就和本来用 JS 也可以实现所有功能，但最后却写 React 的 jsx 或者 Vue 的模板语法一样——为了爽！要想知道有了预处理器有多爽，首先要知道的是传统 CSS 有多不爽。随着前端业务复杂度的提高，前端工程中对 CSS 提出了以下的诉求：
...
![img](./images\2911f98bbacf4b1cbffbb9e1527a4977tplv-k3u1fbpfcp-zoom-in-crop-mark3024000.webp) 它和预处理器的不同就在于，预处理器处理的是 类 CSS，而 PostCss 处理的就是 CSS 本身。Babel 可以将高版本的 JS 代码转换为低版本的 JS 代码。PostCss 做的是类似的事情：它可以编译尚未被浏览器广泛支持的先进的 CSS 语法，还可以自动为一些需要额外兼容的语法增加前缀。更强的是，由于 PostCss 有着强大的插件机制，支持各种各样的扩展，极大地强化了 CSS 的能力。
...
![img](./images\c57fc165a4ce4d5b9a2885867d4f1cabtplv-k3u1fbpfcp-zoom-in-crop-mark3024000.webp)
...
层叠顺序，英文称作 stacking order，表示元素发生层叠时有着特定的垂直显示顺序。下面是盒模型的层叠规则： ![img](./images\fbc59ae1adb5454c8c7f60582df10ff9tplv-k3u1fbpfcp-zoom-in-crop-mark3024000.webp) 对于上图，由上到下分别是： 
...
![img](./images\6523548b65834c578202acc5e6388af4tplv-k3u1fbpfcp-zoom-in-crop-mark3024000.webp)

-   **fixed：** 元素的定位是相对于 window （或者 iframe）边界的，和其他元素没有关系。但是它具有破坏性，会导致其他元素位置的变化。

![img](./images\4b35e646aa3c4d9cac803753d8b67c39tplv-k3u1fbpfcp-zoom-in-crop-mark3024000.webp)

-   **absolute：** 元素的定位相对于前两者要复杂许多。如果为 absolute 设置了 top、left，浏览器会根据什么去确定它的纵向和横向的偏移量呢？答案是浏览器会递归查找该元素的所有父元素，如果找到一个设置了`position:relative/absolute/fixed`的元素，就以该元素为基准定位，如果没找到，就以浏览器边界定位。如下两个图所示：

![img](./images\d6d4f199b3f842f090890eea32318e11tplv-k3u1fbpfcp-zoom-in-crop-mark3024000.webp) ![img](./images\d2c7150bd7614a548f04a2919f1ea12atplv-k3u1fbpfcp-zoom-in-crop-mark3024000.webp)
...
将元素的长宽都设置为 0，显示出来的效果是这样的： ![img](./images\cba8731fea9842a8b8103c2b387fe64ftplv-k3u1fbpfcp-zoom-in-crop-mark3024000.webp) 所以可以根据 border 这个特性来绘制三角形： **（1）三角 1**
...
![img](./images\ab996951a0cc42cf9e6d9e12eb827f8btplv-k3u1fbpfcp-zoom-in-crop-mark3024000.webp) **（2）三角 2**
...
![img](./images\531c6c250dd8446fb0f264e7b3df6fbatplv-k3u1fbpfcp-zoom-in-crop-mark3024000.webp) **（3）三角 3**
...
![img](./images\e4beaf4e4a0140ad9e7252f8a6e4e8e6tplv-k3u1fbpfcp-zoom-in-crop-mark3024000.webp) **（4）三角 4**
...
![img](./images\445f50ad19164b0f863ad8dfef2a29b1tplv-k3u1fbpfcp-zoom-in-crop-mark3024000.webp) **（5）三角 5**
...
![img](./images\a1ac630463164e42a027b54bb95f56batplv-k3u1fbpfcp-zoom-in-crop-mark3024000.webp) 还有很多，就不一一实现了，总体的原则就是通过上下左右边框来控制三角形的方向，用边框的宽度比来控制三角形的角度。
...
![img](./images\db5e46aea0ce4805a0c2bbec2743546etplv-k3u1fbpfcp-zoom-in-crop-mark3024000.webp)
...
打开 Chrome 浏览器，启动移动端调试模式，在控制台去输出这个 `devicePixelRatio` 的值。这里选中 iPhone6/7/8 这系列的机型，输出的结果就是 2： ![img](./images\a8af7dca29f84b7e9d1f94232713ef07tplv-k3u1fbpfcp-zoom-in-crop-mark3024000.webp) 这就意味着设置的 1px CSS 像素，在这个设备上实际会用 2 个物理像素单元来进行渲染，所以实际看到的一定会比 1px 粗一些。 **解决 1px 问题的三种思路：**
